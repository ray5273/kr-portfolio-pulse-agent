...
def config_dir() -> Path:
    return hermes_home() / "config/us-daily-chart-pulse"


def resolve_watchlist() -> Path:
    watchlist_override = os.environ.get("US_WATCHLIST", "").strip()
...
def cli_path() -> Path:
    return hermes_home() / "skills/us-daily-chart-pulse/bin/daily-us-chart-pulse.js"


def output_dir() -> Path:
    return hermes_home() / "artifacts/us-daily-chart-pulse"
...
def build_batches() -> list[dict]:
    ...
    if os.environ.get("US_DRY_RUN", "").strip().lower() in {"1", "true", "yes"}:
        command.append("--dry-run")
    run_date = os.environ.get("US_DATE", os.environ.get("US_RUN_DATE", "")).strip()
    if run_date:
