...
For Telegram image attachments in cron, attach `hermes-send-us-batches.py` as the job script. The script runs the CLI with `--emit-hermes-send-batches`, parses stdout as a JSON array, and calls Hermes' own `send_message` implementation with `target="telegram"` in array order. The message body is `batch.text` followed by three plain `MEDIA:/absolute/path/file.png` lines from `batch.media`, preserving media order.
...
Each successful ticker batch includes `chart.png`, `chart-overlay.png`, and `chart-momentum.png` in that order. The final response should be a short local summary such as `Sent N/M ticker batches`.
